                                  ...
                                  cairo_gl_tex_t      tex_unit)
{
		...
		if(tex_unit == 0)
		{
			...
			if(location != -1)
			{
				if(operand->type == CAIRO_GL_OPERAND_RADIAL_GRADIENT_EXT_NONE_CIRCLE_IN_CIRCLE)
					padding = 0;
				else if(operand->type == CAIRO_GL_OPERAND_RADIAL_GRADIENT_EXT_PAD_CIRCLE_IN_CIRCLE)
					padding = 1;
				else if(operand->type == CAIRO_GL_OPERAND_RADIAL_GRADIENT_EXT_REPEAT_CIRCLE_IN_CIRCLE)
					padding = 2;
				else
					padding = 3;
			...
			_cairo_gl_shader_bind_vec2v(ctx, "source_matrix_1",
		...
		if(tex_unit == 0)
		{
			...
			if(location != -1)
			{
				if(operand->type == CAIRO_GL_OPERAND_RADIAL_GRADIENT_EXT_NONE_CIRCLE_NOT_IN_CIRCLE)
					padding = 0;
				else if(operand->type == CAIRO_GL_OPERAND_RADIAL_GRADIENT_EXT_PAD_CIRCLE_NOT_IN_CIRCLE)
					padding = 1;
				else if(operand->type == CAIRO_GL_OPERAND_RADIAL_GRADIENT_EXT_REPEAT_CIRCLE_NOT_IN_CIRCLE)
					padding = 2;
				else
					padding = 3;
				dispatch->Uniform1i(location, padding);
			...
			_cairo_gl_shader_bind_vec2v(ctx, "source_matrix_1",
				3, operand->gradient.matrix1);
			_cairo_gl_shader_bind_vec2v(ctx, "source_matrix_2",
	...
	*/
    ...
    case CAIRO_GL_OPERAND_LINEAR_GRADIENT_EXT_REFLECT:
		...
		if(tex_unit == 0)
		{
			_cairo_gl_shader_bind_vec2v(ctx, "source_stops", 
				2,
				operand->gradient.stops);
			_cairo_gl_shader_bind_vec4v(ctx, "source_colors",
				operand->gradient.nstops,
		...
		{
			_cairo_gl_shader_bind_vec2v(ctx, "mask_stops", 
				2,
				operand->gradient.stops);
			_cairo_gl_shader_bind_vec4v(ctx, "mask_colors",
				operand->gradient.nstops,
	    ...
	    _cairo_gl_operand_get_gl_filter (operand) == GL_LINEAR)
	{
		...
		// Henry 
		width = operand->texture.width;
		height = operand->texture.height;
	    }
	    else {
					  ...
					  cairo_gl_context_t *ctx)
{
    ...
    cairo_bool_t component_alpha;
    cairo_gl_shader_t *shader;
	float *my_vertices, *my_color;

	my_vertices = (float *)vertices;
	my_color = (float *)color;
    ...
    dst_size  = 2 * sizeof (GLfloat) * vertices_size;
    src_size  = _cairo_gl_operand_get_vertex_size (setup->src.type);
    mask_size = _cairo_gl_operand_get_vertex_size (setup->mask.type);
