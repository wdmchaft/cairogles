                            ...
                            cairo_gl_tex_t name)
{
	...
	*/
	...
	_cairo_output_stream_printf (stream,
		...
		"  float x = %s_r1 * f.x * d / %s_scales[0] + %s_xy1.x;\n"
		"  float y = %s_r1 * f.y * d / %s_scales[1] + %s_xy1.y;\n"
		"  return vec2(x, y);\n"
		"}\n"
		"vec2 %s_get_xy_out_circle(vec2 coord, vec2 xy)\n"
		"{\n"
		"  float x1, x2, y1, y2;\n"
		"  if(%s_circle_2[2] == 0.0)\n"
		"    return %s_circle_2.xy;\n"
		"  float dx = coord.x - %s_circle_1.x;\n"
		"  float dy = coord.y - %s_circle_1.y;\n"
		"  float r2 = %s_circle_2.z / %s_scales[1];\n"
		"  float r1 = %s_circle_1.z / %s_scales[0];\n"
		"  float t;\n"
		"  //if((coord.x - xy.x) / (coord.y - xy.y) != (coord.x - %s_circle_1.x) / (coord.y - %s_circle_1.y)) return vec2(0.0, 0.0);\n"
		"  if(dx == 0.0)\n"
		"  {\n"
		"    x1 = coord.x;\n"
		"    x2 = coord.x;\n"
		"    t = (x1 - %s_circle_2.x) * (x1 - %s_circle_2.x);\n"
		"    t = sqrt(1.0 - t / (r1 * r1)) * r2;\n"
		"    y1 = t + %s_circle_2.y;\n"
		"    y2 = -t + %s_circle_2.y;\n"
		"    //y1 = r2 + %s_circle_2.y;\n"
		"    //y2 = -r2 + %s_circle_2.y;\n"
		...
		"    y1 = coord.y;\n"
		"    y2 = coord.y;\n"
		"    t = (y1 - %s_circle_2.y) * (y1 - %s_circle_2.y);\n"
		"    t = sqrt(1.0 - t / (r2 * r2)) * r1;\n"
		"    x1 = t + %s_circle_2.x;\n"
		"    x2 = -t + %s_circle_2.x;\n"
		"    //x1 = r1 + %s_circle_2.x;\n"
		"    //x2 = -r1 + %s_circle_2.x;\n"
		...
		"    r1 = r1 * r1;\n"
		"    r2 = r2 * r2;\n"
		"    float a = r2 * dx * dx + dy * dy * r1;\n"
		"    float b = -2.0 * r1 * dy * (dy * %s_circle_1.x + dx * %s_circle_2.y - dx * %s_circle_1.y) - 2.0 * dx * dx * r2 * %s_circle_2.x;\n"
		"    float c = r1* (dy * %s_circle_1.x + dx * %s_circle_2.y - dx * %s_circle_1.y) * (dy * %s_circle_1.x + dx * %s_circle_2.y - dx * %s_circle_1.y) + %s_circle_2.x * %s_circle_2.x * r2 * dx * dx - dx * dx * r1 * r2;\n"
		"    //float a = r2 * dx * dx + r1 * dy * dy;\n"
		"    //float b = -2.0 * r2 * dx * (dx * coord.y - dy * coord.x + dy * %s_circle_2.x) - 2.0 * r1 * dy * dy * %s_circle_2.y;\n"
		"    //float c = r2 * (dx * coord.y - dy * coord.x + dy * %s_circle_2.x) * (dx * coord.y - dy * coord.x + dy * %s_circle_2.x) + r1 * dy * dy * %s_circle_2.y * %s_circle_2.y - r1 * r2 * dy * dy;\n"
		"    float sqrtb = sqrt(b * b - 4.0 * a * c);\n"
		"    x1 = (-b + sqrtb) / (2.0 * a);\n"
		"    x2 = (-b - sqrtb) / (2.0 * a);\n"
		"    y1 = dy / dx * (x1 - %s_circle_1.x) + %s_circle_1.y;\n"
		"    y2 = dy / dx * (x2 - %s_circle_1.x) + %s_circle_1.y;\n"
		"  }\n"
		"  //float dis1 = dot((x1 - coord.x), (y1 - coord.y));\n"
		"  //float dis2 = dot((x2 - coord.x), (y2 - coord.y));\n"
		"  float dis1 = (x1 - coord.x) * (x1 - coord.x) + (y1 - coord.y) * (y1 - coord.y);\n"
		"  float dis2 = (x2 - coord.x) * (x2 - coord.x) + (y2 - coord.y) * (y2 - coord.y);\n"
		"  if(dis1 < dis2)\n"
		"    return vec2(x1, y1);\n"
		"  else if(dis2 > dis1)\n"
		"    return vec2(x2, y2);\n"
		...
		"    {\n"
		"      float fx = dx / dy;\n"
		"      float dx1 = x1 - %s_circle_1.x;\n"
		"      float dy1 = y1 - %s_circle_1.y;\n"
		"      float dx2 = x2 - %s_circle_1.x;\n"
		"      float dy2 = y2 - %s_circle_1.y;\n"
		"      float fx1 = dx1 / dy1;\n"
		"      float fx2 = dx2 / dy2;\n"
		"      if(fx > 0.0)\n"
		"      {\n"
		...
		"      float d3 = d31 * d31;\n"
		"      float d4 = sqrt(d2 - d3);\n"
		"      //float d51 = pow(%s_r2, 2.0) - dot(f2, f2);\n"
		"      float d5 = 1.0 /(pow(%s_r2, 2.0) - dot(f2, f2));\n"
		"      float d6 = (d1 + d4) * d5;\n"
		"      return d6;\n"
		"    }\n"
		"  }\n"
		"  else // reflect or repeat extend pattern\n"
		"  {\n"
		"    // need to compute xy of inner circle and outer circle\n"
		"    vec2 inner = %s_get_xy(coord.xy);\n"
		"    vec2 outer = %s_get_xy_out_circle(coord.xy, inner);\n"
		"    float d = %s_get_distance_from_center(inner, outer);\n"
		"    if(%s_pad == 2) // repeat\n"
		"    {\n"
		"      float dis = fract(dis_from_focal / d);\n"
		"      //if(dis_from_focal < %s_circle_1.z)\n"
		"      //  dis += dis_from_focal / %s_circle_1.z;\n"
		"      return dis;\n"
		"    }\n"
		"  }\n"
		"  return 0.0;\n"
		...
		"  %s_r1 = %s_circle_1.z;\n"
		"  %s_r2 = %s_circle_2.z;\n"
		"  %s_xy1.x = %s_circle_1.x;\n"
		"  %s_xy1.y = %s_circle_1.y;\n"
		"  %s_xy2.x = %s_circle_2.x;\n"
		"  %s_xy2.y = %s_circle_2.y;\n"
		"  if(%s_circle_1[2] > %s_circle_2[2])\n"
		"  {\n"
		"    %s_reverse = 1;\n"
		"    %s_r1 = %s_circle_2.z;\n"
		"    %s_r2 = %s_circle_1.z;\n"
		"    %s_xy1.x = %s_circle_2.x;\n"
		"    %s_xy1.y = %s_circle_2.y;\n"
		"    %s_xy2.x = %s_circle_1.x;\n"
