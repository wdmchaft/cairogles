...
typedef struct cairo_gl_shader_impl {
    ...
    void
    (*bind_int) (cairo_gl_context_t *ctx,
		 cairo_gl_shader_t  *shader,
		 const char *name,
		 int value);

    void
    (*bind_float_array) (cairo_gl_context_t *ctx,
			 cairo_gl_shader_t  *shader,
			 const char *name,
			 int num,
			 float *values);
...
static cairo_int_status_t
bind_float_array_core_2_0 (cairo_gl_context_t *ctx,
			   cairo_gl_shader_t *shader,
			   const char *name,
			   int num,
		 	   float *values)
...
static cairo_int_status_t
bind_int_core_2_0 (cairo_gl_context_t *ctx,
		   cairo_gl_shader_t *shader,
		   const char *name,
		   int value)
{
                            ...
                            cairo_gl_tex_t name)
{
    ...
    cairo_extend_t extend = _cairo_gl_operand_get_extend (op);
	...
	_cairo_output_stream_printf (stream,
		...
		"uniform int %s_vertical;\n"
		"uniform float %s_matrix[%d];\n"
		"uniform int %s_radius;\n"
		"uniform float %s_step;\n"
		"varying vec2 %s_texcoords;\n"
		"vec4 get_%s()\n"
		"{\n",
		rectstr, namestr, namestr, namestr, namestr, MAX_FILTER_SIZE,
	    ...
	    _cairo_gl_shader_needs_border_fade (op))
	{
		...
		"    int i;\n"
		"    vec2 coords;\n");
	    if (extend == CAIRO_EXTEND_NONE && ! use_atlas) {
		_cairo_output_stream_printf (stream,
		...
		"        if (%s_vertical == 1)\n"
		"            coords = %s_texcoords + vec2 (0.0, float(i) * %s_step);\n"
		"        else\n"
		"            coords = %s_texcoords + vec2 (float(i) * %s_step, 0.0);\n"
	    ...
	    _cairo_output_stream_printf (stream,
		"        tex += texel * border_fade.x * border_fade.y * %s_matrix[i + %s_radius] ;\n"
		"    }\n"
		"    return tex;\n"
		"}\n",
		namestr, namestr);
	...
	_cairo_output_stream_printf (stream,
		...
		"uniform vec2 %s_texdims;\n"
		"uniform float %s_matrix[%d];\n"
		"uniform int %s_x_radius;\n"
		"uniform float %s_x_step;\n"
		"uniform int %s_y_radius;\n"
		"uniform float %s_y_step;\n"
		"varying vec2 %s_texcoords;\n"
		"vec4 get_%s()\n"
		"{\n",
	    ...
	    _cairo_gl_shader_needs_border_fade (op))
	{
		...
		"        for (j = -%s_y_radius; j <= %s_y_radius; j++) {\n"
		"            coords = %s_texcoords + vec2 (float(i) * %s_x_step, float(j) * %s_y_step);\n"
		"            vec2 border_fade = %s_border_fade (coords, %s_texdims);\n",
		namestr, namestr, namestr, namestr,
		namestr, namestr, namestr, namestr, namestr);
	    if (! use_atlas) {
	    ...
	    _cairo_output_stream_printf (stream,
		"            tex += texel * border_fade.x * border_fade.y * %s_matrix[(i + %s_x_radius) * (2 * %s_x_radius + 1) + j + %s_y_radius] ;\n"
		"        }\n"
		"    }\n"
		"    return tex;\n"
		"}\n",
	...
	else
	{
	    ...
	    _cairo_output_stream_printf (stream,
		"            tex += texel * %s_matrix[(i + %s_x_radius) * (2 * %s_x_radius + 1) + j + %s_y_radius] ;\n"
		"        }\n"
		"    }\n"
		"    return tex;\n"
		"}\n",
...
_cairo_gl_shader_bind_int (cairo_gl_context_t *ctx,
			   ...
			   int value)
{
    ctx->shader_impl->bind_int (ctx, ctx->current_shader, name, value);
}
...
_cairo_gl_shader_bind_float_array (cairo_gl_context_t *ctx,
				   ...
				   float *values)
{
    ctx->shader_impl->bind_float_array (ctx, ctx->current_shader, name, num, values);
}
void
