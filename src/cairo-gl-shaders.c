                            ...
                            cairo_gl_tex_t name)
{
	...
	*/
	...
	_cairo_output_stream_printf (stream,
		...
		"  return vec2(x, y);\n"
		"}\n"
		"vec2 %s_get_xy_out_circle(vec2 coord, int angle)\n"
		"{\n"
		"  float x1, x2, y1, y2;\n"
		"  float dx = coord.x - %s_xy1.x;\n"
		"  float dy = coord.y - %s_xy1.y;\n"
		"  float r2 = %s_r2 / %s_scales[1];\n"
		"  float r1 = %s_r2 / %s_scales[0];\n"
		"  float t;\n"
		"  if(dx == 0.0)\n"
		"  {\n"
		"    x1 = coord.x;\n"
		"    t = pow((x1 - %s_xy2.x), 2.0);\n"
		...
		"    r2 = pow(r2, 2.0);\n"
		"    float a, b, c;\n"
		"    a = r2 * pow(dx, 2.0) + pow(dy, 2.0) * r1;\n"
		"    if(angle == 0)\n"
		"    {\n"
		"      b = -2.0 * r1 * dy * (dy * %s_xy1.x + dx * %s_xy2.y - dx * %s_xy1.y) - 2.0 * dx * dx * r2 * %s_xy2.x;\n"
		"      c = r1 * pow((dy * %s_xy1.x + dx * %s_xy2.y - dx * %s_xy1.y), 2.0) + %s_xy2.x * %s_xy2.x * r2 * dx * dx - dx * dx * r1 * r2;\n"
		"    }\n"
		"    else\n"
		"    {\n"
		"      b = -2.0 * r2 * dx * (dx * %s_xy1.y + dy * %s_xy2.x - dy * %s_xy1.x) - 2.0 * dy * dy * r1 * %s_xy2.y;\n"
		"      c = r2 * pow((dx * %s_xy1.y + dy * %s_xy2.x - dy * %s_xy1.x), 2.0) + %s_xy2.y * %s_xy2.y * r1 * dy * dy - dy * dy * r1 * r2;\n"
		"    }\n"
		"    if(b * b - 4.0 * a * c < 0.0) return vec2(-1.0, -1.0);\n"
		"    float sqrtb = sqrt(b * b - 4.0 * a * c);\n"
		"    if(angle == 0)\n"
		"    {\n"
		"      x1 = (-b + sqrtb) / (2.0 * a);\n"
		"      x2 = (-b - sqrtb) / (2.0 * a);\n"
		"      //y1 = dy / dx * (x1 - %s_xy1.x) + %s_xy1.y;\n"
		"      //y2 = dy / dx * (x2 - %s_xy1.x) + %s_xy1.y;\n"
		"      y1 = dy / dx * (x1 - coord.x) + coord.y;\n"
		"      y2 = dy / dx * (x2 - coord.x) + coord.y;\n"
		"    }\n"
		"    else\n"
		"    {\n"
		"      y1 = (-b + sqrtb) / (2.0 * a);\n"
		"      y2 = (-b - sqrtb) / (2.0 * a);\n"
		"      x1 = dx / dy * (y1 - coord.y) + coord.x;\n"
		"      x2 = dx / dy * (y2 - coord.y) + coord.x;\n"
		"    }\n"
		"  }\n"
		"  if(dx > 0)\n"
		"  {\n"
		...
		"    int angle = 0;\n"
		"    float dx = %s_xy1.x - coord.x;\n"
		"    float dy = %s_xy1.y - coord.y;\n"
		"    if(dy == 0.0 && dx == 0.0)\n"
		"    {\n"
		"      coord.x += 0.01 * %s_r1 / %s_scales[0];\n"
		"      angle = 0;\n"
		"    }\n"
		"    else if(dy == 0.0)\n"
		"      angle = 0;\n"
		...
		"    else\n"
		"      angle = 1;\n"
		"    vec2 outer = %s_get_xy_out_circle(coord.xy, angle);\n"
		"    //float small_circle_distance = %s_get_distance_from_center(inner, %s_xy1);\n"
		"    float d = %s_get_distance_from_center(inner, outer);\n"
		"    //float n = (dis_from_focal - small_circle_distance) / d;\n"
		"    float n = (dis_from_focal - %s_r1) / d;\n"
		"    //if(n < 0.0) return 0.0;\n"
		"    float dis = fract(n);\n"
		"    if(dis < 0.0)\n"
		"      dis += 1.0;\n"
		...
		"  }\n"
		"  %s_dis = %s_get_distance_from_start(coord);\n"
		"  if(%s_dis == -1.0) return vec4(1.0, 0.0, 0.0, 1.0);\n"
		"  if(%s_reverse == 1)\n"
		"    %s_dis = 1.0 - %s_dis;\n"
		...
		"  }\n"
		"/*\n"
		"  if(%s_dis <= 0)  return vec4(0.5, 0.0, 0.0, 1.0);\n"
		"  else if(%s_dis <= 0.5) return vec4(1.0, 0.0, 0.0, 1.0);\n"
		"  else if(%s_dis <= 1.0) return vec4(0.0, 1.0, 0.0, 1.0);\n"
		"  else  return vec4(0.0, 0.0, 0.5, 1.0);\n"
		"*/\n"
		"  if(%s_nstops == 2)\n"
		"  {\n"
		"    %s_relative_dis = %s_dis - %s_offsets[0];\n"
